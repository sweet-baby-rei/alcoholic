...
def calculate(
    ...
    pos_price: Annotated[
        Optional[float],
        typer.Argument(metavar="PRICE", help="Total price (Positional)"),
    ] = None,
    pos_quantity: Annotated[
        Optional[float],
        typer.Argument(metavar="QUANTITY", help="Volume quantity (Positional)"),
    ] = None,
    # 2. KEYWORD Options (For explicit/legacy usage)
    opt_price: Annotated[
        Optional[float], typer.Option("--price", "-p", help="Total price (Flag)")
    ] = None,
    opt_quantity: Annotated[
        Optional[float], typer.Option("--quantity", "-q", help="Volume quantity (Flag)")
    ] = None,
    # 3. Standard Overrides
    abv: Annotated[
        Optional[float],
        typer.Option("--abv", "-a", help="Alcohol percentage (e.g., 13.5)"),
    ] = None,
    unit: Annotated[
        Optional[str], typer.Option("--unit", "-u", help="Override default volume unit")
    ] = None,
    currency: Annotated[
        Optional[str],
        typer.Option("--currency", "-c", help="Currency code (e.g., USD, JPY, GBP)"),
    ] = None,
    offline: Annotated[
        bool, typer.Option("--offline", help="Skip live API conversion")
    ] = False,
    # 4. Smart Drink Flags
    beer: Annotated[
        bool, typer.Option("--beer", help="Evaluate as Beer (Defaults 5% ABV, 0.5L)")
    ] = False,
    cider: Annotated[
        bool, typer.Option("--cider", help="Evaluate as Cider (Defaults 5% ABV, 0.5L)")
    ] = False,
    wine: Annotated[
        bool, typer.Option("--wine", help="Evaluate as Wine (Defaults 13% ABV, 0.75L)")
    ] = False,
    mid_strength: Annotated[
        bool,
        typer.Option(
            "--liqueur",
            "--sake",
            "--soju",
            "--port",
            help="Evaluate as Liqueur (Defaults 20% ABV, 0.7L)",
        ),
    ] = False,
    spirit: Annotated[
        bool,
        typer.Option(
            ...
            "--whiskey",
            "-s",
            help="Evaluate as Spirit (Defaults 40% ABV, 0.7L)",
        ),
    ] = False,
):
    """
    ...
    if price is None:
        console.print(
            "[bold red]Error:[/bold red] You must provide a price (e.g., `alcoholic 20` or `alcoholic --price 20`)."
        )
    ...
    active_currency = (
        currency if currency else config.get("active_currency", "USD")
    ).upper()

    if active_unit not in VOLUME_UNITS:
        console.print(
            f"[bold red]{lang.get('error_unit', 'Error')}:[/bold red] {active_unit}"
        )
        raise typer.Exit(code=1)
    ...
    if final_abv is None or final_qty is None:
        console.print(
            "[bold red]Error:[/bold red] You must provide a Quantity and ABV, OR use a smart drink flag (e.g., --wine)."
        )
        raise typer.Exit(code=1)

    if final_qty <= 0 or final_abv <= 0:
        console.print(
            "[bold red]Error:[/bold red] Quantity and ABV must be greater than zero."
        )
        raise typer.Exit(code=1)
    ...
    if active_currency in CURRENCY_THRESHOLDS:
        ...
        symbol = str(currency_data.get("symbol", f"{active_currency} "))
        raw_thresholds = currency_data.get(category, currency_data.get("generic"))
        thresholds = cast(dict[str, float], raw_thresholds)
    else:
    ...
    abv_display = f"{final_abv:.1f}% ABV" + (
        " [dim](inferred)[/dim]" if abv is None else ""
    )
    qty_display = f"{final_qty:.2f} {active_unit}" + (
        " [dim](inferred)[/dim]" if quantity is None else ""
    )

    table.add_row(
        "Receipt Summary:", f"{qty_display} @ {abv_display} for {symbol}{price:.2f}"
    )
    table.add_row("Pure Ethanol Volume:", f"{pure_vol:.3f} {active_unit}")
    table.add_row(
        f"Cost per Pure {active_unit}:",
        f"[bold green]{symbol}{local_cost_per_pure:.2f}[/bold green]",
    )

    if active_currency not in CURRENCY_THRESHOLDS:
        table.add_row(
            "[dim]Evaluated against:[/dim]", f"[dim]USD Base (${eval_cost:.2f})[/dim]"
        )
